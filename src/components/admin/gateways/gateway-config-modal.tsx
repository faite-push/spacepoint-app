          ...
          <div className="flex items-center justify-between rounded-md border border-white/5 bg-white/[0.02] p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Credenciais de homologação</p>
              </div>
            </div>
            <Switch
              checked={formData.sandbox === true}
              onCheckedChange={(val) => onFormChange({ ...formData, sandbox: val })}
            />
          </div>
          ...
          {template.fields.map((field) => (
            <div key={field.key} className="space-y-2">
              <Label className="text-sm font-medium text-white/80">
                {field.label}
                {formData.sandbox === false ? " — Produção" : " — Sandbox"}
              </Label>
              ...
              {field.type === "file" ? (
                ...
                >
                  ...
                  />
                  {(formData[field.key] || hasStoredCertificate) && (
